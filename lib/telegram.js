...
// Пост с карточкой. caption — готовый текст (заголовок, выжимка, источник,
// ссылка). parseMode ('HTML' и т.п.) опционален; при нём длину контролирует
// вызывающий (обрезка могла бы разорвать тег). Возвращает result с message_id.
export async function sendPhoto(chatId, pngBuffer, caption, parseMode) {
  const form = new FormData();
  form.append('chat_id', String(chatId));
  form.append('caption', parseMode ? caption : caption.slice(0, CAPTION_LIMIT));
